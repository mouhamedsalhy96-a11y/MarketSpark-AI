...
import { Sparkles, Package, Target, AlignLeft, Copy, CheckCircle2 } from "lucide-react";
      ...
      <div className="w-full md:w-[400px] lg:w-[500px] bg-white border-r border-zinc-200 p-8 flex flex-col h-screen sticky top-0 overflow-y-auto">
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h1 className="text-xl font-black tracking-tight text-zinc-900">MarketSpark AI</h1>
          </div>
          <p className="text-sm text-zinc-500 font-medium">Generate high-converting marketing copy in seconds.</p>
        </div>
      ...
      <div className="flex-1 bg-[#F8FAFC] p-8 md:p-12 overflow-y-auto h-screen">
        {!results && !loading && (
          <div className="h-full flex flex-col items-center justify-center text-zinc-400 space-y-4">
            ...
            <p className="text-sm font-medium">Fill out the form on the left to generate copy.</p>
          </div>
        )}
        ...
        {loading && (
          <div className="h-full flex flex-col items-center justify-center text-indigo-600 space-y-4 animate-pulse">
            ...
            <p className="text-sm font-bold tracking-widest uppercase">Writing your content...</p>
          </div>
        )}
          ...
          <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="bg-white border border-zinc-200 rounded-xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-zinc-100">
                <h3 className="text-sm font-black uppercase tracking-widest text-zinc-800">LinkedIn Post</h3>
                <button onClick={() => handleCopy(results.linkedin, "linkedin")} className="text-xs font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1">
                  {copied === "linkedin" ? <CheckCircle2 className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                  {copied === "linkedin" ? "Copied!" : "Copy"}
                </button>
              </div>
              <p className="text-sm text-zinc-700 leading-relaxed whitespace-pre-wrap">{results.linkedin}</p>
            </div>

            <div className="bg-white border border-zinc-200 rounded-xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-zinc-100">
                <h3 className="text-sm font-black uppercase tracking-widest text-zinc-800">Twitter / X</h3>
                <button onClick={() => handleCopy(results.twitter, "twitter")} className="text-xs font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1">
                  {copied === "twitter" ? <CheckCircle2 className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                  {copied === "twitter" ? "Copied!" : "Copy"}
                </button>
              </div>
              <p className="text-sm text-zinc-700 leading-relaxed whitespace-pre-wrap">{results.twitter}</p>
            </div>

            <div className="bg-white border border-zinc-200 rounded-xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-zinc-100">
                <h3 className="text-sm font-black uppercase tracking-widest text-zinc-800">Email Newsletter</h3>
                <button onClick={() => handleCopy(results.email, "email")} className="text-xs font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1">
                  {copied === "email" ? <CheckCircle2 className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                  {copied === "email" ? "Copied!" : "Copy"}
                </button>
              </div>
              <p className="text-sm text-zinc-700 leading-relaxed whitespace-pre-wrap">{results.email}</p>
            </div>
